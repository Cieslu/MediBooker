...
namespace MediBookerAPI.Controllers
{
    ...
    public class ModeratorController : ControllerBase, IModeratorController
    {
        ...
        [HttpGet("LoadModerator/{id}")]
        public async Task<ActionResult<WorkerDTO>> LoadModerator(string id)
        {
            try
            {
                WorkerDTO? moderator = await _moderatorService.GetModerator(id);
                return Ok(moderator);
            }
            catch
            {
        ...
        /* [HttpPut("ChangePassword/{id}")]
         public async Task<ActionResult> ChangePassword(string id, ChangePasswordDTO changePassword)
         {
             try
             {
                 ...
                 {
                     Boolean result = await _moderatorService.EditPassword(id!, changePassword);
